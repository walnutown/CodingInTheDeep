...
public class ch9_9_2 {
   ...
   public static void findPath(int[][] grid, int row, int colum, ArrayList<int[][]> res){
      ...
      if ( row+1 < grid.length && grid[row+1][colum] != 1)
         findPath(grid, row+1, colum, res);
      if (colum+1 < grid[0].length && grid[row][colum+1] != 1)
         findPath(grid, row, colum+1, res);
      grid[row][colum] = 0;
   } 
}


// Actually, we can directly use formula to solve the first question
// Num = C(X, X+Y) = (X+Y)!/(X!Y!)
